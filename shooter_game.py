...
# Create a group of asteroid sprites()
...
real_time = False # variable untuk reload
num_fire = 0 # variabel to count shots
...
while run:
    for e in event.get():
        ...
        # event untuk menembak menggunakan spasi
            ...
            if e.key == K_SPACE:
                if num_fire < 20 and real_time == False:
                    num_fire = num_fire + 1
                    rocket.fire()

                    if num_fire >= 20 and real_time == False: #if the player fired 5 shot
                        last_time = timer()
                        real_time = True
    ...
    if not finish:
        ...
        # set different color depend life
        if life == 3:
            life_color = (1,150,0)
        if life == 2:
